#' args1 = feature-annotated vcf (*.feature.dummies.pon.score)
...
fwrite(x = f, file = paste0(args[1], '.re_trained'), sep = '\t', row.names = T, col.names = T)
...
if (length(args) == 3){
  ...
  write_tsv(f, paste0(args[1] %>% gsub('.feature.dummies.pon.score', '', .), '.re_trained.', round(threshold, 3), '.vcf'))
}
