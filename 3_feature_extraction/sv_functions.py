...
def set_constants(refver):
	...
	satellite_path = os.path.dirname(os.path.realpath(__file__))
	if refver == '19':
		satellite_path = satellite_path + '/satellite_hg19_rename_slop_subt_sort.bed.gz'
		primary_contigs = [ str(x) for x in list(range(1, 23)) + ['X', 'Y'] ]
	elif refver == '38':
		satellite_path = satellite_path + '/satellite_hg38_slop_subt_sort.bed.gz'
