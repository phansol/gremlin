...
#' args1 = *.gremlin.feature.dummies.pon
...
sampleid <- args[1] %>% basename() %>% gsub('.gremlin.feature.dummies.pon', '', .)

# GREMLIN
gremlin <- readRDS(paste0(dirname(dirname(srcdir)), '/gremlin.fit.rds'))
...
write_tsv(tier1, paste0(outdir, '/', sampleid, '.gremlin.somatic.svs.tier1.tsv'))
write_tsv(tier2, paste0(outdir, '/', sampleid, '.gremlin.somatic.svs.tier2.tsv'))
